...
import { saveCampaignaudience, updateAudience } from "../../views/api/Api.jsx";

const CampaignAudienceModal = (props) => {  // Renamed to start with uppercase
  const { isOpen, toggle, audience: initialaudience, callback, brandId: propBrandId } = props;
  const context = useViewContext();
  const brandId = propBrandId || context?.brandId;
  ...
  const submitAudience = async () => {
    ...
    if (!formData.id) {
      const response = await saveCampaignaudience(payload);
...
export default CampaignAudienceModal; // Update export name accordingly