...
const Form = () => {
  ...
  const btnClicked = (e) => {
    switch (e.target.id) {
      case "searchNext":
        if (startPoint.current.value && endPoint.current.value && ticketType.current.value && journeyDate.current.value) {
          if (startPoint.current.value === endPoint.current.value) {
            err_1.current.innerText = "* Arrival point can't be same as Departure";
          } else {
          ...
          err_1.current.innerText = "* Please fill all the details before proceeding";
        }
        break;
      case "bookTickets":
        if (availTrainGrid.current.getSelectedRecords() === undefined || availTrainGrid.current.getSelectedRecords().length === 0) {
          err_2.current.innerText = "* Select your convenient train";
        } else {
      ...
      case "confirmTickets":
        ...
        if (name.value === "" || age === "" || gender === "") {
          err_3.current.innerText = "* Please enter passenger details";
        } else {
  ...
  const finalizeDetails = (args) => {
    ...
    for (let i = 1; i <= 3; i++) {
      if (name1 !== "") {
        const details = {};
        const gender = i === 1 ? pass_gender1.current.value : i === 2 ? pass_gender2.current.value : pass_gender3.current.value;
        const berth = i === 1 ? pass_berth1.current.value : i === 2 ? pass_berth2.current.value : pass_berth3.current.value;
        details.TrainNo = selectedTrain.TrainNo.toString();
        details.PassName = i === 1 ? name1 : i === 2 ? name2 : name3;
    ...
    for (const city of cities) {
      if (startPoint.current.value === city.name) calcFare += city.fare;
      if (endPoint.current.value === city.name) calcFare += city.fare;
    }
  ...
  const content1 = () => {
    return (
      <div id="selectTrain">
        ...
        <GridComponent ref={availTrainGrid} width="100%" rowSelected={trainSelected}>
          <ColumnsDirective>
  ...
  const content3 = () => {
    return (
      <div id="payment">
        <div className="details-page wizard-title">Make Payment</div>
        <div>
          <div className="details-page" ref={amount} />
        </div>
        <div className="btn-container">
          <button
            id="makePayment"
  ...
  return (
    <div className="control-section">
      ...
      <TabComponent ref={tab} selected={tabSelected}>
        <TabItemsDirective>
