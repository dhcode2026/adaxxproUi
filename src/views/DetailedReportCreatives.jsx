...
import AdsCustomizationModal from "./customizationcolumns/AdsCustomizationModal";
...
const DetailedReportCreatives = (props) => {
    ...
    return (
      <Dropdown isOpen={dropdownOpen} toggle={toggle}>
        ...
        <DropdownMenu>
          ...
          <DropdownItem onClick={() => navigate(`/admin/group/${encodeURIComponent(row.name)}/campaigns/${row.groupId}`, {
            state: {
              name: row.name,
              groupId: row.groupId,
              groups: row.groups
  ...
  const ALL_COLUMNS = {
    ...
    "Ad Name": {
      ...
      cell: (row) => <div className="gOorhn">{row.adName || "-"}</div>,
      sortable: true,
      width: "150px",
    },
    "Internal Review": {
      name: "Internal Review",
      selector: (row) => row.internalReview || "-",
      cell: (row) => <div className="gOorhn">{row.internalReview || "-"}</div>,
      sortable: true,
      width: "150px",
    },
    "External Review": {
      name: "External Review",
      selector: (row) => row.externalReview || "-",
      cell: (row) => <div className="gOorhn">{row.externalReview || "-"}</div>,
      sortable: true,
      width: "150px",
    },
    "Ad Type": {
      ...
      cell: (row) => <div className="gOorhn">{row.adType || "-"}</div>,
      sortable: true,
      width: "120px",
    },
    "Preview": {
      ...
      cell: (row) => <div className="gOorhn">{row.preview || "-"}</div>,
      sortable: true,
      width: "120px",
    },
    "Destination URL": {
      ...
      cell: (row) => <div className="gOorhn">{row.destinationUrl || "-"}</div>,
      sortable: true,
      width: "200px",
    },
    "Imp. URL": {
      name: "Imp. URL",
      selector: (row) => row.impUrl || "-",
      cell: (row) => <div className="gOorhn">{row.impUrl || "-"}</div>,
      sortable: true,
      width: "200px",
    },
    "Primary Conv": {
  ...
  const steps = [
    { label: "Daily Reporting", icon: "icon-chart-bar-32" },
    { label: "Hourly Reporting", icon: "icon-time-alarm" },
  ];
  ...
  const editgroup = async (id) => {
    ...
    try {
      ...
      let groupData;
      if (response.data?.data?.informationGroups) {
        groupData = response.data.data.informationGroups[0];
      } else if (response.data?.informationGroups) {
        groupData = response.data.informationGroups[0];
      ...
      if (groupData) {
        const formattedGroup = {
          id: groupData.groupId || groupData.id || id,
          groupId: groupData.groupId || groupData.id || id,
          name: groupData.name || "Unnamed Group",
          notes: groupData.notes || "",
          status: groupData.status,
          initialStatus: groupData.initialStatus,
          budgetType: groupData.budgetType,
          budget: groupData.budget,
          impressionCap: groupData.impressionCap,
          impressionCaps: groupData.impressionCaps,
          pacing: groupData.pacing,
          groupBudgetOptimization: groupData.groupBudgetOptimization,
          frequencyCap: groupData.frequencyCap,
          advertiserSpend: groupData.advertiserSpend,
          rate: groupData.rate,
          maxAmount: groupData.maxAmount,
          footfall: groupData.footfall,
          cubicCampaign: groupData.cubicCampaign,
          reach: groupData.reach,
          kpimetric: groupData.kpimetric,
          kpivalue: groupData.kpivalue,
          startDate: groupData.startDate,
          endDate: groupData.endDate,
          createdAt: groupData.createdAt,
          updatedAt: groupData.updatedAt,
          brandId: groupData.brandId || currentBrandId,
          ...groupData
        };
        console.log("Setting group to modal:", formattedGroup);
  ...
  //         kpivalue: item.kpivalue || item.kpivalue || "-",
  //         brandId: item.brandId || currentBrandId,
  //         // Add dummy data for new columns
  //         impressionsWon: Math.floor(Math.random() * 10000),
  //         winRate: `${Math.floor(Math.random() * 100)}%`,
  //         advSpendECPM: `$${(Math.random() * 10).toFixed(2)}`,
  //         totalECPM: `$${(Math.random() * 12).toFixed(2)}`,
                    ...
                    <Row className="align-items-center">
                      ...
                      <Col xs="auto" className="">
                        <Button
                          ...
                          className="form-control py-1 px-1 rounded-0 adsheight custom-select-input"
                          id="export"
                          onClick={toggleAdsCustomizationModal}>
...
export default DetailedReportCreatives;