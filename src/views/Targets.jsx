  ...
  const getTargetsView = () => {
    ...
    return targets.map((row, index) => (
      <tr key={"targetsview-" + row}>
        <td>{index}</td>
        <td key={"targets-name-" + index} className="text-left">
          {row.name}
        </td>
        <td key={"targets-camp" + index} className="text-left">
          {vx.getCampaignNameByTargetId(row.id)}
        </td>
        {vx.user.sub_id === "superuser" && (
          <td key={"targets-cust-" + index} className="text-left">
            {row.customer_id}
          </td>
        )}
        <td key={"targets-id-" + index} className="text-right">
          {row.id}
        </td>
        <td className="text-center">
          <Button color="success" size="sm" onClick={() => editTarget(row.id)}>
            Edit
          </Button>
          &nbsp;
          <Button
              ...
              <>
                <div className="row mb-3">
                  <div className="col-xl-12 col-lg-12">
                      ...
                      <Col xs="12" className="m-0 p-0">
                        <div className="d-flex justify-content-center m-0 p-0">
                          <strong className="h5 w-100 text-center border border-1 py-1 mb-1">
                            <i className="tim-icons icon-check-2 me-2"></i>
                            Targets
                          </strong>
                        </div>
                      </Col>
                    ...
                    <Table key={"bidders-table-" + count} className="table table-bordered table-hover w-100 h-100">
