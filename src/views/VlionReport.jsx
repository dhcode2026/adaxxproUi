...
const VlionReport = () => {
  ...
  const exchangeName = rawExchangeName || "vlion";
  const {
  ...
  const getPresetRange = (preset) => {
    ...
    switch (preset) {
      ...
      case "2 Days Ago": {
        ...
        return {
          startDate: new Date(twoDaysAgo.getFullYear(), twoDaysAgo.getMonth(), twoDaysAgo.getDate()),
          endDate: new Date(twoDaysAgo.getFullYear(), twoDaysAgo.getMonth(), twoDaysAgo.getDate(), 23, 59, 59, 999)
        };
      }
      case "Last 7 Days": {
  ...
  const fetchReportData = useCallback(async () => {
    ...
    try {
      ...
      const mappedData = rawData.map((item) => {
        // FIX: Use createdAt as fallback when reportDate is null
        const reportDate = item.reportDate || item.createdAt || "";
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      format: (row) => formatWithComma(row.clicks)
    },
  ];
                      ...
                      <Col md="2" className="p-0">
                        <div className="position-relative ms-2">
                          <Input
                            ...
                            type="text"
                            placeholder="Search"
                            style={{ fontSize: "0.685rem", height: "30px", borderRadius: "4px" }}
                            value={searchTerm}
